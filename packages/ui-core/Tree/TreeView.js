...
import { SvgUse } from 'src/ui/SvgIcon'
import TreeViewContext from './TreeViewContext'
import { DescendantProvider } from './descendants'
import styles from './TreeView.mdl.css'
...
const TreeView = React.forwardRef((props, ref) => {
  const {
    children,
    className,
    defaultCollapseIcon = <SvgUse name="minus" size="xs" />,
    defaultEndIcon,
    defaultExpanded = defaultDefaultExpanded,
    defaultExpandIcon = <SvgUse name="plus" size="xs" />,
    defaultParentIcon,
  ...
  const getChildrenIds = (id) => Object.keys(nodeMap.current)
  ...
  const registerNode = React.useCallback((node) => {
    const {
      id, index, parentId, expandable, idAttribute, disabled,
    } = node;
