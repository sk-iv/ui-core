...
import {
  setRef, useEventCallback, useControlled, unstable_useId as useId,
} from '../utils'
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (value !== null && !freeSolo && options.length > 0) {
      ...
      if (missingValue.length > 0) {
        console.warn(
          [
            `SivaSifr-UI: The value provided to ${componentName} is invalid.`,
    ...
    if (multiple) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (matches.length > 1) {
          console.error(
            [
              `SivaSifr-UI: The \`isOptionEqualToValue\` method of ${componentName} do not handle the arguments correctly.`,
      ...
      switch (event.key) {
        case 'Home':
          if (popupOpen && handleHomeEndKeys) {
            ...
            changeHighlightedIndex({
              diff: 'start', direction: 'next', reason: 'keyboard', event,
            });
          }
          break;
        case 'End':
        ...
        case 'PageDown':
          ...
          event.preventDefault();
          changeHighlightedIndex({
            diff: pageSize, direction: 'next', reason: 'keyboard', event,
          });
          handleOpen(event);
          break;
        case 'ArrowDown':
      ...
      if (acc.length > 0 && acc[acc.length - 1].group === group) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          if (indexBy.get(group) && !warn) {
            console.warn(
              `SivaSifr-UI: The options provided combined with the \`groupBy\` method of ${componentName} returns duplicated headers.`,
