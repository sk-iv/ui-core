...
const ChipInput = React.forwardRef(function ChipInput(props, ref) {
  const {
    allowDuplicates,
  ...
  const [focusedChip, setFocusedChip] = React.useState(null);
  ...
  const updateChips = (chips, additionalUpdates = {}) => {
    setChips(chips);
    if (onChange) {
      onChange(chips);
    }
  ...
  // const actualInputValue = inputValue !== "" ? inputValue : ""

  const hasInput = (value || actualInputValue).length > 0 || actualInputValue.length > 0;
    ...
    return chipRenderer(
      {
        ...
        className: 'chip',
        color: (isActive.length && isActive.some((el) => el === value)) ? 'accent' : 'default',
      },
      i,
    );
  });
  ...
  return (
    ...
    >
      ...
      <InputComponent
        ...
        readOnly={readOnly}
        style={{ flexWrap: 'wrap' }}
        startAdornment={<InputAdornment style={{ flexWrap: 'wrap', height: 'auto', maxHeight: 'inherit' }} position="start">{chipComponents}</InputAdornment>}
        {...InputProps}
        {...InputMore}
      />
...
export const defaultChipRenderer = ({
  value, text, isFocused, isDisabled, isReadOnly, handleClick, handleDelete, className, color = 'primary',
}, key) => (
  <Chip
    key={key}
    className={className}
    color={isFocused || color ? color : 'default'}
